...
function library:window(properties)
	local cfg = {
		name = properties.Name or properties.name or properties.Title or properties.title or "avidware",
		size = properties.Size or properties.size or dim2(0, 620, 0, 460),
	}

	local animated_text = library:animation(cfg.name .. " | dev")

	-- watermark (fixed height 24px, dynamic automatic width, no clipping)
	...
	task.spawn(function()
		while true do
			if __holder and __holder.Visible then
	...
	-- window frame layout (vertical left navigation sidebar + modern header)
	local inline1 = library:create("Frame", {
		Parent = library.gui,
		Name = "MainWindow",
		Active = true,
	...
	local build_label = library:create("TextLabel", {
		Parent = top_bar,
		Name = "BuildBadge",
		FontFace = library.font,
		TextColor3 = Color3.fromRGB(120, 120, 140),
		Text = properties.Build or properties.build or "v1.0",
		TextStrokeTransparency = 0.5,
		BorderSizePixel = 0,
		BackgroundTransparency = 1,
	...
	local sidebar = library:create("Frame", {
		...
		Size = UDim2.new(0, 136, 1, -45),
		BorderColor3 = Color3.fromRGB(36, 36, 46),
		BorderSizePixel = 1,
		BackgroundColor3 = Color3.fromRGB(18, 18, 24),
	})

	local sidebar_padding = library:create("UIPadding", {
		Parent = sidebar,
		Name = "",
	...
	local sidebar_list = library:create("UIListLayout", {
		Parent = sidebar,
		Name = "",
	...
	cfg["tab_holder"] = sidebar

	-- Main Right Content Area
	local content_holder = library:create("Frame", {
		Parent = inline1,
		Name = "ContentArea",
		Position = UDim2.new(0, 150, 0, 37),
	...
	cfg["tab_instance_holder"] = content_holder
	...
	-- global chat window (separate menu sub-window)
	local chat_window = library:create("Frame", {
		Parent = library.gui,
		Name = "GlobalChatSubWindow",
		Active = true,
		Draggable = true,
		Visible = false,
		Position = UDim2.new(0, WINDOW_PATH.AbsolutePosition.X + WINDOW_PATH.AbsoluteSize.X + 8, 0, WINDOW_PATH.AbsolutePosition.Y + 1),
		BorderColor3 = Color3.fromRGB(8, 8, 8),
		Size = UDim2.new(0, 320, 0, 340),
		BackgroundColor3 = Color3.fromRGB(56, 56, 56),
	})
	library:make_resizable(chat_window)
	table.insert(library.main_frame, chat_window)

	local chat_title = library:create("TextLabel", {
		Parent = chat_window,
		Name = "",
		FontFace = library.font,
		TextColor3 = Color3.fromRGB(170, 170, 170),
	...
	local chat_main = library:create("Frame", {
		Parent = chat_window,
		Name = "",
		Position = UDim2.new(0, 4, 0, 4),
		BorderColor3 = Color3.fromRGB(26, 26, 26),
	...
	local chat_tabs = library:create("Frame", {
		Parent = chat_main,
		Name = "",
		Position = UDim2.new(0, 8, 0, 8),
		BorderColor3 = Color3.fromRGB(8, 8, 8),
	...
	local chat_scroller = library:create("ScrollingFrame", {
		Parent = chat_tabs,
		Name = "",
		Position = UDim2.new(0, 6, 0, 6),
	...
	library:apply_theme(chat_scroller, "accent", "ScrollBarImageColor3")

	library:create("UIListLayout", {
		Parent = chat_scroller,
		Name = "",
		Padding = UDim.new(0, 4),
		SortOrder = Enum.SortOrder.LayoutOrder,
	})
	...
	local chat_input_bar = library:create("Frame", {
		Parent = chat_tabs,
		Name = "",
	...
	local chat_msg_input = library:create("TextBox", {
		Parent = chat_input_bar,
		Name = "",
		FontFace = library.font,
		TextColor3 = Color3.fromRGB(170, 170, 170),
		...
		Size = UDim2.new(1, -54, 1, -4),
		TextXAlignment = Enum.TextXAlignment.Left,
	})

	local chat_send_btn = library:create("TextButton", {
		Parent = chat_input_bar,
		Name = "",
		FontFace = library.font,
		TextColor3 = Color3.fromRGB(170, 170, 170),
		...
		Size = UDim2.new(0, 40, 1, -4),
		BorderColor3 = Color3.fromRGB(56, 56, 56),
		BackgroundColor3 = Color3.fromRGB(38, 38, 38),
	})

	local chat_accent_line = library:create("Frame", {
		Parent = chat_window,
		Name = "",
		BorderColor3 = Color3.fromRGB(34, 34, 34),
		Size = UDim2.new(1, 0, 0, 2),
	...
	library:apply_theme(chat_accent_line, "accent", "BackgroundColor3")

	local chat_glow = library:create("ImageLabel", {
		Parent = chat_window,
		Name = "",
		ImageColor3 = themes.preset.accent,
		ScaleType = Enum.ScaleType.Slice,
	...
	library:apply_theme(chat_glow, "accent", "ImageColor3")

	local chat_last_msg_ids = {}

	local function add_chat_message_ui(sender, content, created_at)
		local time_str = created_at and string.sub(tostring(created_at), 12, 16) or os.date("%H:%M")

		local msg_card = library:create("Frame", {
			Parent = chat_scroller,
			Name = "",
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 0),
		...
		local sender_lbl = library:create("TextLabel", {
			Parent = msg_card,
			Name = "",
			FontFace = library.font,
		...
		library:apply_theme(sender_lbl, "accent", "TextColor3")

		local time_lbl = library:create("TextLabel", {
			Parent = msg_card,
			Name = "",
			FontFace = library.font,
			...
			TextSize = 10,
			BackgroundTransparency = 1,
			Position = UDim2.new(0, sender_lbl.TextBounds.X + 4, 0, 1),
			AutomaticSize = Enum.AutomaticSize.XY,
		})

		local content_lbl = library:create("TextLabel", {
			Parent = msg_card,
			Name = "",
			FontFace = library.font,
			TextColor3 = Color3.fromRGB(170, 170, 170),
		...
		chat_scroller.CanvasPosition = Vector2.new(0, 99999)
	end

	local function fetch_chat_messages_sub()
		if not library.supabase_url or library.supabase_url == "" then
			return
		end
		local res = library:supabase_request("messages?select=*&order=created_at.asc&limit=40", "GET")
		if type(res) == "table" then
			for _, item in ipairs(res) do
				if item.id and not chat_last_msg_ids[item.id] then
					chat_last_msg_ids[item.id] = true
					add_chat_message_ui(item.username or item.sender_id or "user", item.content or "", item.created_at)
				end
			end
		end
	end

	local function send_chat_message_sub()
		local text = chat_msg_input.Text
		if text and text:gsub("%s+", "") ~= "" then
			chat_msg_input.Text = ""
			local author = lp and lp.Name or "guest"
			local payload = {
				username = author,
				content = text,
			}
			library:supabase_request("messages", "POST", payload)
			task.delay(0.3, fetch_chat_messages_sub)
		end
	end

	chat_send_btn.MouseButton1Click:Connect(send_chat_message_sub)
	chat_msg_input.FocusLost:Connect(function(enter_pressed)
		if enter_pressed then
			send_chat_message_sub()
		end
	end)

	task.spawn(function()
		while true do
			if chat_window.Visible then
				fetch_chat_messages_sub()
			end
			task.wait(2.5)
		end
	end)
	...
	function cfg.toggle_chat(bool)
		chat_window.Visible = bool
		flags["chat_window"] = bool
		if bool then
			fetch_chat_messages_sub()
		end
	end
	...
	function cfg.set_menu_visibility(bool, pl)
		...
		chat_window.Visible = flags["chat_window"] and bool or false

		-- Ensure Roblox player chat remains accessible when menu is open
		pcall(function()
			game:GetService("StarterGui"):SetCoreGuiEnabled(Enum.CoreGuiType.Chat, true)
		end)
	end

	-- Allow '/' key to focus/open player chat even when menu is open
...
function library:tab(properties)
	...
	-- Sidebar Tab Button (Vertical Left Navigation)
	local TAB_BUTTON = library:create("TextButton", {
		Parent = self.tab_holder,
		Name = cfg.name .. "_Button",
		FontFace = library.font,
	...
	local button_content = library:create("Frame", {
		Parent = TAB_BUTTON,
		Name = "Content",
		Position = UDim2.new(0, 8, 0, 0),
	...
	local button_text = library:create("TextLabel", {
		...
		TextXAlignment = Enum.TextXAlignment.Left,
		TextSize = 12,
		Size = UDim2.new(1, cfg.icon and -24 or 0, 1, 0),
	})
	cfg["text_instance"] = button_text

	-- Tab Main Frame Container
	...
	local subpage_bar = library:create("Frame", {
		Parent = TAB,
		Name = "SubPageBar",
		Position = UDim2.new(0, 6, 0, 6),
	...
	local subpage_layout = library:create("UIListLayout", {
		Parent = subpage_bar,
		Name = "",
		FillDirection = Enum.FillDirection.Horizontal,
	...
	local function create_column_pair(parent_frame, y_offset, height_sub)
		local left = library:create("ScrollingFrame", {
			...
			AutomaticCanvasSize = Enum.AutomaticSize.Y,
			ScrollBarThickness = 2,
			Size = UDim2.new(0.5, -6, 1, height_sub or 0),
			BackgroundTransparency = 1,
			Position = UDim2.new(0, 2, 0, y_offset or 0),
			BorderSizePixel = 0,
			CanvasSize = UDim2.new(0, 0, 0, 0),
		})
		...
		local right = library:create("ScrollingFrame", {
			...
			Size = UDim2.new(0.5, -6, 1, height_sub or 0),
			BackgroundTransparency = 1,
			Position = UDim2.new(0.5, 4, 0, y_offset or 0),
			BorderSizePixel = 0,
			CanvasSize = UDim2.new(0, 0, 0, 0),
		})
	...
	local left, right = create_column_pair(scrolling_columns, 0, 0)
	cfg["left"] = left
	cfg["right"] = right

	-- Method to create SubPages inside a Page/Tab
	function cfg:SubPage(sub_props)
		...
		local SUB_BUTTON = library:create("TextButton", {
			Parent = subpage_bar,
			Name = sub_name .. "_SubButton",
			FontFace = library.font,
			TextColor3 = themes.preset.unselected_text,
			BorderColor3 = Color3.fromRGB(0, 0, 0),
			Text = sub_name,
		...
		local sub_indicator = library:create("Frame", {
			Parent = SUB_BUTTON,
			Name = "Indicator",
		...
		library:apply_theme(sub_indicator, "accent", "BackgroundColor3")

		local sub_left, sub_right = create_column_pair(scrolling_columns, 0, 0)
		sub_left.Visible = false
		sub_right.Visible = false
		...
		function sub_obj:Section(section_props)
			return library:section({
				name = section_props.name or section_props.Name,
				side = section_props.side or section_props.Side or "left",
				holder = sub_obj,
			})
		end
		sub_obj.section = sub_obj.Section

		function sub_obj:open()
			for _, other_sub in ipairs(cfg.subpages) do
				other_sub.button.TextColor3 = themes.preset.unselected_text
				other_sub.indicator.Visible = false
				other_sub.left.Visible = false
				other_sub.right.Visible = false
			end

			sub_obj.button.TextColor3 = themes.preset.text
			sub_obj.indicator.Visible = true
			sub_obj.left.Visible = true
			sub_obj.right.Visible = true
			cfg.current_subpage = sub_obj
		end

		SUB_BUTTON.MouseButton1Click:Connect(function()
			sub_obj:open()
		end)

		table.insert(cfg.subpages, sub_obj)
		if #cfg.subpages == 1 then
			sub_obj:open()
		end

		return sub_obj
	end
	cfg.subpage = cfg.SubPage
	cfg.Page = cfg
	cfg.page = cfg
	cfg.Section = function(self_obj, sec_props)
		return library:section({
			name = sec_props.name or sec_props.Name,
			side = sec_props.side or sec_props.Side or "left",
			holder = cfg,
		})
	end
	...
	function cfg.open_tab()
		if library.current_tab and library.current_tab[1] ~= TAB_BUTTON then
			local old_button = library.current_tab[1]
			old_button.BackgroundTransparency = 1
			local old_ind = old_button:FindFirstChild("Indicator")
			if old_ind then old_ind.Visible = false end
			
			local old_text = old_button:FindFirstChild("Content") and old_button.Content:FindFirstChild("Text")
			if old_text then old_text.TextColor3 = themes.preset.unselected_text end

			local old_icon = old_button:FindFirstChild("Content") and old_button.Content:FindFirstChild("Icon")
			if old_icon then old_icon.ImageColor3 = themes.preset.unselected_text end
		...
		button_text.TextColor3 = themes.preset.text
		if cfg.icon_instance then
			cfg.icon_instance.ImageColor3 = themes.preset.accent
		end
	...
	-- If first tab created, automatically open it
...
function library:section(properties)
	local holder = properties.holder or self
	local side = (properties.side or properties.Side or "left"):lower()
	local parent_column = holder[side] or (side == "right" and holder.right or holder.left) or self.left
	...
	cfg["holder"] = elements
	cfg["bottom_holder"] = elements
	cfg["right_holder"] = elements

	-- Method aliases on Section
	cfg.Toggle = function(s, p) p = p or {}; p.holder = elements; return library:toggle(p) end
	cfg.toggle = cfg.Toggle
	cfg.Slider = function(s, p) p = p or {}; p.holder = elements; return library:slider(p) end
	cfg.slider = cfg.Slider
	cfg.Dropdown = function(s, p) p = p or {}; p.holder = elements; return library:dropdown(p) end
	cfg.dropdown = cfg.Dropdown
	cfg.Colorpicker = function(s, p) p = p or {}; p.holder = elements; return library:colorpicker(p) end
	cfg.colorpicker = cfg.Colorpicker
	cfg.Keybind = function(s, p) p = p or {}; p.holder = elements; return library:keybind(p) end
	cfg.keybind = cfg.Keybind
	cfg.Button = function(s, p) p = p or {}; p.holder = elements; return library:button(p) end
	cfg.button = cfg.Button
	cfg.Textbox = function(s, p) p = p or {}; p.holder = elements; return library:textbox(p) end
	cfg.textbox = cfg.Textbox
	cfg.HitpartPicker = function(s, p) p = p or {}; p.holder = elements; return library:hitpart_picker(p) end
	cfg.hitpart_picker = cfg.HitpartPicker
...
function library:keybind(properties)
	...
	library:connection(uis.InputBegan, function(input, game_event)
		if not game_event and not uis:GetFocusedTextBox() then
			if input.UserInputType == Enum.UserInputType.Keyboard then
...
function library:Watermark(name_text, icon_id)
	name_text = name_text or "avidware"
	local wm_obj = {
		text = name_text,
		icon = icon_id,
		visible = true,
	...
	if icon_id then
		...
		library:apply_theme(icon_img, "accent", "ImageColor3")
	end
	...
	function wm_obj:SetText(t)
		wm_obj.text = t
		label.Text = t
	end

	function wm_obj:SetVisible(v)
		wm_obj.visible = v
		holder.Visible = v
	end

	function wm_obj:Destroy()
		holder:Destroy()
	end
...
function library:KeybindList()
	return {
		SetVisible = function(self, v)
			if library.keybind_path and library.keybind_path.Parent then
				local kblist = library.keybind_path:FindFirstAncestorOfClass("Frame") or library.keybind_path.Parent
				if kblist then kblist.Visible = v end
			end
		end,
	}
end
...
function library:CreateSettingsPage(window, keybind_list, watermark)
	...
	config_sec:Button({
		Text = "Save Config",
		Callback = function()
			local cfg_name = flags["Config_Name_Input"] or "default"
			if cfg_name ~= "" then
				writefile(library.directory .. "/configs/" .. cfg_name .. ".cfg", library:get_config())
				library:config_list_update()
				library:notification({ text = "Saved config: " .. cfg_name })
			end
		end,
	})
	...
	config_sec:Button({
		...
		Callback = function()
			local cfg_name = flags["Config_Name_Input"] or "default"
			local path = library.directory .. "/configs/" .. cfg_name .. ".cfg"
			if isfile(path) then
				library:load_config(readfile(path))
				library:notification({ text = "Loaded config: " .. cfg_name })
			else
				library:notification({ text = "Config not found: " .. cfg_name })
			end
		end,
	})
	...
	menu_sec:Toggle({
		...
		Default = true,
		Callback = function(v)
			if keybind_list and keybind_list.SetVisible then
				keybind_list:SetVisible(v)
			end
		end,
	})
	...
	menu_sec:Toggle({
		...
		Default = false,
		Callback = function(v)
			if not library.global_chat then
	...
	local theme_toggle = menu_sec:Toggle({
		Name = "Accent Color",
		Default = true,
		Callback = function() end,
	})

	theme_toggle:Colorpicker({
		Flag = "Menu_Accent_Color",
...
library.create_settings_page = library.CreateSettingsPage

-- Supabase REST API Service
library.supabase_url = library.supabase_url or ""
library.supabase_key = library.supabase_key or ""

function library:supabase_request(endpoint, method, data)
	if not library.supabase_url or library.supabase_url == "" then
		return nil
	end

	local url = library.supabase_url .. "/rest/v1/" .. endpoint
	local headers = {
		["apikey"] = library.supabase_key,
		["Authorization"] = "Bearer " .. library.supabase_key,
		["Content-Type"] = "application/json",
		["Prefer"] = "return=representation",
	}

	local req_fn = (syn and syn.request) or (http and http.request) or http_request or request or (fluxus and fluxus.request)
	local body_json = data and http_service:JSONEncode(data) or nil

	if req_fn then
		local response = req_fn({
			Url = url,
...
function library:chat(properties)
	properties = properties or {}
	local cfg = {
		title = properties.title or "Global Chat",
		visible = false,
	}

	local chat_frame = library:create("Frame", {
		Parent = library.gui,
		Name = "SupabaseChatWindow",
		Active = true,
		Draggable = true,
		Visible = false,
	...
	library:make_resizable(chat_frame)
	table.insert(library.main_frame, chat_frame)
	...
	local msg_scroller = library:create("ScrollingFrame", {
		Parent = chat_frame,
	...
	local input_bar = library:create("Frame", {
		Parent = chat_frame,
		Name = "InputBar",
	...
	local msg_input = library:create("TextBox", {
		Parent = input_bar,
		Name = "Input",
		FontFace = library.font,
		TextColor3 = Color3.fromRGB(220, 220, 230),
	...
	local send_btn = library:create("TextButton", {
		Parent = input_bar,
		Name = "SendBtn",
		FontFace = library.font,
		TextColor3 = Color3.fromRGB(220, 220, 230),
	...
	local function add_message_ui(sender, content, created_at)
		local time_str = created_at and string.sub(tostring(created_at), 12, 16) or os.date("%H:%M")
		...
		msg_scroller.CanvasPosition = Vector2.new(0, 99999)
	end

	local function fetch_chat_messages()
		if not library.supabase_url or library.supabase_url == "" then
			return
		end
		local res = library:supabase_request("messages?select=*&order=created_at.asc&limit=40", "GET")
		if type(res) == "table" then
			for _, item in ipairs(res) do
				if item.id and not last_msg_ids[item.id] then
					last_msg_ids[item.id] = true
					add_message_ui(item.username or item.sender_id or "User", item.content or "", item.created_at)
				end
			end
		end
	end
	...
	local function send_chat_message()
		...
		if text and text:gsub("%s+", "") ~= "" then
			...
			library:supabase_request("messages", "POST", payload)
			task.delay(0.3, fetch_chat_messages)
		end
	end

	send_btn.MouseButton1Click:Connect(send_chat_message)
	msg_input.FocusLost:Connect(function(enter_pressed)
		if enter_pressed then
			send_chat_message()
		end
	end)

	function cfg:SetVisible(v)
		cfg.visible = v
		chat_frame.Visible = v
		if v then
			fetch_chat_messages()
		end
	end

	-- Auto polling loop when chat window is open
	task.spawn(function()
		while true do
